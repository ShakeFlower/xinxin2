var plugins_bb40132b_638b_4a9f_b028_d3fe47acc8d1 = 
{
	...
	*/
	...
	class GameCore {
		...
		canExecAction(action) {
			...
			if (action === 'q')
				...
				// 攻小于等于敌人防御，剑技和会心不能使用
				if (action === 'b' && Math.round(1.5 * (this.hero.atk - this.enemy.def)) < this.hero.lv)
					return { success: false, reason: '当前对敌人能造成的伤害太低，无法发动凡骨' };
		...
		// 攻防后增加气息
		addMana(actor, damage) {
			switch (actor) {
			case 'hero':
				if (this.enemy.def >= this.hero.atk) return;
				if (this.swordSkill.length > 0) {
					//使用剑技或C的回合中勇士将没有攻增气
				} else {
				...
				if (core.hasEquip('I319') && (this.swordSkill === 'c') || this.swordSkill === 's') {
					// 装备流石用C不加气息
				} else {
				...
				break;
			case 'enemy':
				if (core.hasSpecial(this.special, 60)) { //暗魔法无防增气
				} else {
		...
		enemyAct() {
			...
			if (this.enemy.freeze > 0) {
				...
				if (core.hasSpecial(especial, 2) || core.hasSpecial(especial, 61) ||
				...
				if (this.enemy.mana >= this.enemy.manamax) { //满气息释放必杀
			...
			case 'C': //结晶盾
				...
				if (combo >= 2) {
					if (this.turn % (combo + 1) === 1) this.enemy.freeze = combo - 1;
					else this.enemy.freeze = 2 * combo - this.turn % (combo + 1);
				} else this.enemy.freeze = 1;
			...
			case 'F': //精灵罩
				this.hero.fairy = 3;
				this.hero.fairyBuff = (this.hero.hp + this.hero.def) % this.hero.lv;
			...
			if (!atkStatus.freeze && !atkStatus.miss) {
				...
				if (core.hasSpecial(especial, 58) && atkStatus.crit) {
					this.hero.mana -= this.hero.permana;
					if (this.hero.mana < 0) this.hero.permana = 0;
				}
				...
				if (!atkStatus.crit) this.addMana('enemy', damage); //本回合未暴击则回复气息
				if (this.hero.fairy > 0) {
					atkStatus.fairy = true;
					this.hero.fairy--;
					if (this.hero.fairy === 0) {
						this.hero.def -= Math.round(this.hero.def / 50) + (this.hero.hp % this.hero.lv);
	...
	function drawBattleUI(battleInfo) {
		...
		core.fillText(ctx, hero.atk + atkBuff, width - tx - 54, ty + textFontSize + lineHeight + 2, atkColor, numFont);
		core.fillText(ctx, hero.def + defBuff, width - tx - 54, ty + 2 * (textFontSize + lineHeight) + 2, defColor, numFont);
		...
		// 绘制气息条
		function drawMana() {
			const enemyManaRatio = enemy.mana / enemy.manamax,
	...
	this.test = function (enemyFrame = 0) {
		drawBattleUI({ 'hero': { 'hp': 1000, 'atk': 10, 'def': 10, 'fatigue': 0, 'mana': 240, 'manamax': 240 }, 'enemy': { 'id': "greenSlime", 'hp': 10, 'atk': 10, 'def': 10, 'fatigue': 0, 'mana': 50, 'manamax': 50 } }, enemyFrame);
		drawSkillIcon();
	}
	...
	function battleByTurn(id, x, y) {
		...
		function takeTurn() {
			...
			if (battle.turn % (combo + 1) === 1) {
				...
				if (atkStatusH.freeze) {} else {
					...
					drawBattleUI(battle);
					if (currDelay > 100) {
						if (!atkStatusH.freeze) core.plugin.drawAnimateByPixel(atkStatusH.animate, 60, 152);
		...
		if (core.isReplaying()) {
			...
			core.registerAnimationFrame('battleIcon', true, function (timestamp) {
				...
				drawIconCount++;
				timeCount = timestamp;
				drawBattleIcon(battle, drawIconCount);
			});
			drawBattleUI(battle);
			drawSkillIcon(battle);
